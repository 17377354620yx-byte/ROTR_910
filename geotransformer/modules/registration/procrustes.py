...
def weighted_procrustes(
    ...
    # Adding eps to a positive mass shrinks both centroids and makes the pose
    # depend on arbitrary confidence units (coarse scores can be very small).
    mass = torch.sum(weights, dim=1, keepdim=True)
    weights = weights / mass.clamp_min(torch.finfo(weights.dtype).tiny)
    ...
    eye = torch.eye(3, dtype=src_points.dtype, device=src_points.device).unsqueeze(0).repeat(batch_size, 1, 1)
    ...
    if return_transform:
        transform = torch.eye(4, dtype=src_points.dtype, device=src_points.device).unsqueeze(0).repeat(batch_size, 1, 1)
        transform[:, :3, :3] = R
        transform[:, :3, 3] = t
        if squeeze_first:
