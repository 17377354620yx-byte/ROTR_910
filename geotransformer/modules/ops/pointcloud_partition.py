...
def point_to_node_partition(
    ...
    actual_limit = min(point_limit, points.shape[0])
    node_knn_indices = sq_dist_mat.topk(k=actual_limit, dim=1, largest=False)[1]  # (M, K')
    node_knn_node_indices = index_select(point_to_node, node_knn_indices, dim=0)  # (M, K)
    node_indices = torch.arange(nodes.shape[0], device=nodes.device).unsqueeze(1).expand(-1, actual_limit)
    ...
    if actual_limit < point_limit:
        padding = point_limit - actual_limit
        sentinel_indices = torch.full(
            (nodes.shape[0], padding),
            points.shape[0],
            dtype=node_knn_indices.dtype,
            device=node_knn_indices.device,
        )
        padding_masks = torch.zeros(
            (nodes.shape[0], padding), dtype=torch.bool, device=node_knn_masks.device
        )
        node_knn_indices = torch.cat([node_knn_indices, sentinel_indices], dim=1)
        node_knn_masks = torch.cat([node_knn_masks, padding_masks], dim=1)
